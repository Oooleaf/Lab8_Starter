describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    //because first item already clicked, here start from 1 
    ...
    const cnt = await page.$('#cart-count');
    const innerText = await cnt.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    //also make sure #car-count is still 20
    let cnt = await page.$('#cart-count');
    let innerText = await cnt.getProperty('innerText');
    let text = await innerText.jsonValue();
    expect(text).toBe('20');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    //make sure that #cart-item is now 0
    let cnt = await page.$('#cart-count');
    let innerText = await cnt.getProperty('innerText');
    let text = await innerText.jsonValue();
    expect(text).toBe('0');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    //also make sure #car-count is still 0
    let cnt = await page.$('#cart-count');
    let innerText = await cnt.getProperty('innerText');
    let text = await innerText.jsonValue();
    expect(text).toBe('0');
